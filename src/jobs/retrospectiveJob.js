...
請只回傳 JSON: {"decision": "PASS" 或 "VETO", "reason": "50字內的審查意見"}`;
            ...
            if (report.target_prompt_id && report.new_prompt_content && report.target_prompt_id !== "null") {
                if (isVetoed) {
                    ...
                    const isIllegalUpdate = (!hasTrades && report.target_prompt_id === 'reviewer_bluechip' && !hasBluechipTrade && false); // 若無交易，放寬容許修改
                    if (isIllegalUpdate) {
                        promptUpdateLog = `❌ 系統攔截：拒絕越權修改！`;
                    } else {
            ...
            // 👇👇👇 完美支援 jsonb 寫入，將 prompt_changes 包裝成 Object
