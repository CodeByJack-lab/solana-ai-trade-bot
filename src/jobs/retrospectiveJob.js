...
// 🚀 將所有 Gemini Keys 放入 Array (自動過濾空值)
const GEMINI_KEYS = [
...
const retrospectiveJob = {
    ...
    async runEvolutionWithRetry(attempt = 1) {
        ...
        try {
            ...
            const hasMemeTrade = [...badTrades, ...bestTrades].some(t => (t.strategy_type || '').includes('MEME'));
            const hasTrendingTrade = [...badTrades, ...bestTrades].some(t => (t.strategy_type || '').includes('TRENDING'));
            ...
            const { data: param3 } = await supabase.from('ai_strategy_params').select('*').eq('id', 3).single(); // ID 3: Trending
            ...
            // 🚀 核心升級：教導 Master AI 三線作戰的設定與要求
            promptText += `\n\n【重要系統設定說明】\n系統目前有三套獨立參數 (請根據虧損單的 strategy 類型調整對應參數)：\n`;
            promptText += `ID 1 (老幣/BLUECHIP專用): min_liquidity=${param1?.min_liquidity}, min_vol_5m=${param1?.min_vol_5m}, bluechip_max_rsi=${param1?.bluechip_max_rsi}, bluechip_min_drop_pct=${param1?.bluechip_min_drop_pct}\n`;
            promptText += `ID 2 (新幣/MEME盲狙專用): min_liquidity=${param2?.min_liquidity}, min_vol_5m=${param2?.min_vol_5m}\n`;
            promptText += `ID 3 (熱門榜/TRENDING專用): min_liquidity=${param3?.min_liquidity}, min_vol_5m=${param3?.min_vol_5m}\n`;
            ...
            promptText += `"recommended_params": { "bluechip": { "min_liquidity": 20000, "bluechip_max_rsi": 40 }, "meme": { "min_liquidity": 6000 }, "trending": { "min_liquidity": 40000 } }`;

            const { data: currentPrompts } = await supabase.from('bot_prompts').select('*');
            if (currentPrompts) {
                let contextStr = "\n\n【當前系統使用的 AI 劇本 (僅提供有包含該策略好壞單的部門供你修改)】\n";
                if (hasMemeTrade || hasTrendingTrade) {
...
請只回傳 JSON: {"decision": "PASS" 或 "VETO", "reason": "50字內的審查意見"}`;
            ...
            if (!isVetoed && report.recommended_params) {
                ...
                if (bcParams) {
                    const bcUpdates = {};
                    if (bcParams.min_liquidity !== undefined) bcUpdates.min_liquidity = Math.max(10000, Math.min(Number(bcParams.min_liquidity), 100000));
                    if (bcParams.min_vol_5m !== undefined) bcUpdates.min_vol_5m = Math.max(1000, Math.min(Number(bcParams.min_vol_5m), 20000));
                    if (bcParams.bluechip_max_rsi !== undefined) bcUpdates.bluechip_max_rsi = Math.max(20, Math.min(Number(bcParams.bluechip_max_rsi), 50));
                    if (bcParams.bluechip_min_drop_pct !== undefined) bcUpdates.bluechip_min_drop_pct = Math.max(1, Math.min(Number(bcParams.bluechip_min_drop_pct), 10));
                ...
                if (report.recommended_params.meme) {
                    ...
                    const memeUpdates = {};
                    if (memeParams.min_liquidity !== undefined) memeUpdates.min_liquidity = Math.max(3000, Math.min(Number(memeParams.min_liquidity), 30000));
                    if (memeParams.min_vol_5m !== undefined) memeUpdates.min_vol_5m = Math.max(500, Math.min(Number(memeParams.min_vol_5m), 15000));
                ...
                // 3. 🚀 爆升熱門幣 (Trending - ID 3)
                if (report.recommended_params.trending) {
                    const trendParams = report.recommended_params.trending;
                    const trendUpdates = {};
                    // 防呆限制：Top50 幣流動性下限鎖死最少 1萬，最高 15萬
                    if (trendParams.min_liquidity !== undefined) trendUpdates.min_liquidity = Math.max(10000, Math.min(Number(trendParams.min_liquidity), 150000));
                    if (trendParams.min_vol_5m !== undefined) trendUpdates.min_vol_5m = Math.max(2000, Math.min(Number(trendParams.min_vol_5m), 50000));
...
📝 <b>AI劇本進化</b>: ${promptUpdateLog}
    ...
    start() {
        ...
        console.log(`🤖 [Evolution] 雙軌審查進化排程已啟動 (支援三部門聯合進化)...`);
    }
};
