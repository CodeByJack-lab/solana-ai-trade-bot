...
// 🔐 初始化錢包 (用於支付手動 Burn 的 Gas，並接收退回的租金)
let wallet;
try {
    const rawKey = process.env.SOLANA_PRIVATE_KEY;
    if (rawKey) {
        wallet = Keypair.fromSecretKey(
            rawKey.startsWith('[') ? Uint8Array.from(JSON.parse(rawKey)) : bs58.decode(rawKey)
        );
    }
} catch (e) {
    console.error("❌ [GraveyardJob] 錢包初始化失敗，無法執行火化。");
}
...
const graveyardJob = {
        ...
        try {
            ...
            for (const token of deadTokens) {
                ...
                try {
                    ...
                    console.log(`🔥 [Graveyard] 火化成功！${token.token_symbol} 已消滅，0.002 SOL 租金已回流主錢包。`);
    ...
    start() {
        ...
        console.log('🕒 [GraveyardJob] 墓地火化排程已啟動 (每晚凌晨 3 點執行)');
    }
};
