...
'mistral-large-latest',
            ...
            if (decision.action === 'SELL_HALF' || decision.action === 'SELL_ALL') {
                ...
                if (pos) {
                    ...
                    if (acquired) {
                        const fraction = decision.action === 'SELL_HALF' ?
0.5 : 1.0;
...
6000) { 
    ...
    if (deadMints.length > 0) {
        ...
        try {
            ...
            const res = 
await axios.get(`https://api.jup.ag/price/v3?ids=${jupMints.join(',')}`, { timeout: 3000 });
            
            const solUsd = parseFloat(res.data?.data?.['So11111111111111111111111111111111111111112']?.price ||
'1');
...
setInterval(async () => {
    ...
    try {
        // 🚀 核心修復：拔除 5 分鐘時光陷阱，直接獲取所有 INCUBATING 代幣
        ...
        // 🎯 判斷是否滿足出車條件
        const oneMinAgoTs = Date.now() - 60 * 1000;
        const oldestTokenTs = new Date(candidates[0].created_at).getTime();
        const isTimeoutReached = oldestTokenTs <= oneMinAgoTs;

        let tokensToProcess = [];

        if (candidates.length >= 20 || isTimeoutReached) {
            const processCount = Math.floor(candidates.length / 20) * 20;
            const finalCount = processCount === 0 ? candidates.length : processCount;
            tokensToProcess = candidates.slice(0, finalCount);
            console.log(`\n⏱️ [Incubator] 觸發條件 (滿20或逾時1分鐘)！提取 ${tokensToProcess.length} 隻開車查價...`);
        } else {
            return;
        }
            ...
            try {
                ...
                for (const token of batch) {
                    ...
                    const marketData = {
                        ...
                        b: pair.txns?.m5?.buys ||
0,
                        s: pair.txns?.m5?.sells ||
0,
                        l: pair.liquidity?.usd ||
0,
                        ts: Date.now(),
...
0; 
        ...
        const envState = envStateStr ?
JSON.parse(envStateStr) : { climate: 'CHOPPY' };
        ...
        try {
            ...
            if (!llmTargetData.description || llmTargetData.description.trim() === '') {
                llmTargetData.description = `Newly launched community token.
Ticker: $${llmTargetData.symbol}, Name: ${llmTargetData.name}. No official description provided yet. Please evaluate the viral/meme potential based solely on its ticker and name.
Do NOT penalize for lacking description.`;
            }
        ...
        try {
            ...
            if (mlParamsStr) {
                ...
                // 確保能解析 Array 格式
                const paramsArray = Array.isArray(mlParams) ?
mlParams : [];
        ...
        if (finalScore >= buyThreshold) {
            ...
            if (acquired) {
                ...
                try {
                    ...
                    if (config) {
                        baseAmount = poolType === 'NEWBORN' ?
config.trade_amount_sol : config.trending_trade_amount_sol;
                    }
                } catch(e){}
        ...
        const symbol = symbol_cache.get(mint) ||
'UNKNOWN';
        ...
        try {
            ...
            if (incubating) {
                ...
                if (pair && pair.priceUsd && pair.liquidity?.usd >= 5000) {
                    ...
                    // 🚀 核心修復：打包完整數據供綠色通道使用
                    const marketData = {
                        p: parseFloat(pair.priceUsd), v: pair.volume?.m5 ||
0,
                        b: pair.txns?.m5?.buys ||
0, s: pair.txns?.m5?.sells || 0,
                        l: pair.liquidity?.usd ||
0, ts: Date.now(), 
                        description: pair.info?.description ||
pair.baseToken?.name || '',
